...
def set_ucsc_gene_file(genome_id, gene_model):
    ...
    if genome_id == "hg19":
        if gene_model == "refseq":
            ucsc_gene_file = pkg_resources.resource_filename("annot_utils2", "data/hg19/refGene.txt.gz")
        elif gene_model == "gencode":
            ucsc_gene_file = pkg_resources.resource_filename("annot_utils2", "data/hg19/wgEncodeGencodeBasicV19.txt.gz")
            # ucsc_gene_file = pkg_resources.resource_filename("annot_utils2", "data/hg19/wgEncodeGencodeCompV31lift37.txt.gz")
    elif genome_id == "hg38":
        if gene_model == "refseq":
            ucsc_gene_file = pkg_resources.resource_filename("annot_utils2", "data/hg38/refGene.txt.gz")
        elif gene_model == "gencode":
            # ucsc_gene_file = pkg_resources.resource_filename("annot_utils2", "data/hg38/wgEncodeGencodeBasicV24.txt.gz")
            ucsc_gene_file = pkg_resources.resource_filename("annot_utils2", "data/hg38/wgEncodeGencodeCompV31.txt.gz")
    elif genome_id == "mm10":
        if gene_model == "refseq":
            ucsc_gene_file = pkg_resources.resource_filename("annot_utils2", "data/mm10/refGene.txt.gz")
        elif gene_model == "gencode":
            # ucsc_gene_file = pkg_resources.resource_filename("annot_utils2", "data/mm10/wgEncodeGencodeBasicVM9.txt.gz")
            ucsc_gene_file = pkg_resources.resource_filename("annot_utils2", "data/mm10/wgEncodeGencodeCompVM22.txt.gz")
    else:
